...
$plugin['name'] = 'gds_groupdocs_assembly';
...
$plugin['version'] = '0.1';
$plugin['author'] = 'Pavel Teplitskiy';
...
# --- BEGIN PLUGIN CODE ---
/**
 * GroupDocs Assembly Plugin v0.1.0
...
if (@txpinterface == 'admin') {
    ...
    register_callback('gds_append_button', 'article_ui', 'title');
}

// add GroupDocs button
function gds_append_button($event, $step, $data, $rs) {

    $js= gds_javascript();
    $button = '<input type="button" value="Insert GroupDocs Composer ID" onclick="gds_insert_fileid()">';
    $output_result = isset($rs['url_title']) ? '<br/>' . $js.$button : '';
    return $data.$output_result;
}

// add javascript
function gds_javascript(){
    // jquery is working here
    $r= '<script>';
    $r.= 'function gds_insert_fileid(){
